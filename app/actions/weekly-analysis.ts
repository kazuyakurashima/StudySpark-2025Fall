...
export async function getWeeklyStudyData(studentId: string, weekStartDate: Date, weekEndDate: Date) {
  ...
  studyLogs?.forEach((log: any) => {
    ...
    subjectStats[subjectName].correctAnswers += log.correct_count
...
/**
 * バッチ処理用: 管理者権限でAI分析を生成
 * （Supabase Admin Clientを使用するため、RLSをバイパス可能）
 */
export async function generateWeeklyAnalysisForBatch(studentId: string, weekStartDate: Date, weekEndDate: Date) {
  // 管理者クライアントを作成
  ...
  // AI分析プロンプトを構築
  const analysisData = {
    study: { subjectSummary, totalStudyDays: studyLogs?.length || 0 },
    encouragement: { stats: encouragementStats },
