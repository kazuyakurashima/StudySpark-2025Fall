...
import { getChildPastExamResults } from "@/app/actions/past-exam"
...
export function ParentPastExamViewer({ childId, childName }: Props) {
  ...
  const loadResults = async () => {
    ...
    const response = await getChildPastExamResults(childId)
      ...
      {stats ? (
        ...
        <Card className="card-elevated border-0 shadow-md">
          <CardContent className="py-10 text-center text-muted-foreground">
            ...
            <p className="text-lg font-medium mb-2">まだ過去問の結果がありません</p>
            <p className="text-sm">
              {childName}さんが過去問演習の結果を入力すると、<br />
              こちらに表示されます
            </p>
          </CardContent>
        </Card>
