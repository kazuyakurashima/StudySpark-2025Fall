...
import { Calendar, Flag, Save, Bot, Sparkles, Target, PartyPopper } from "lucide-react"
import { GoalNavigationChat } from "./goal-navigation-chat"
import { getAvailableTests, saveTestGoal } from "@/app/actions/goal"
...
export default function NewGoalPage() {
  ...
  useEffect(() => {
    const loadTests = async () => {
      const { tests, error } = await getAvailableTests()
      if (tests && !error) {
      ...
      <div className="surface-gradient-primary backdrop-blur-lg border-b border-border/30 p-3 sm:p-4 shadow-lg">
        <div className="max-w-4xl mx-auto">
          <h1 className="text-lg sm:text-xl font-bold text-foreground flex items-center gap-2">
            ...
            ゴールナビ
            {studentGrade && <span className="text-sm text-muted-foreground ml-2">(小学{studentGrade}年生)</span>}
          </h1>
                      ...
                      {availableTests.map((test) => (
                        <SelectItem key={test.id} value={test.id}>
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-muted-foreground">{formatDate(test.test_date)}</span>
                          </div>
                        </SelectItem>
                      ))}
                    </SelectContent>
                ...
                {selectedTest && (
                  <div className="mt-4 p-3 surface-gradient-primary rounded-lg border border-primary/20">
                    ...
                    <p className="text-xs text-muted-foreground">
                      種類: {selectedTest.test_types.type_category === "gohan" ? "合不合判定テスト" : "公開組分けテスト"}
                    </p>
                  </div>
                )}
              </CardContent>
