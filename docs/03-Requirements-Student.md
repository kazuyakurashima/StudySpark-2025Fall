...
* **復習促進モード**：**火 / 木 / 土**
  ...
  * 科目ごとの3パネルで右上に状況を表示
    ...
    * 未入力の場合は、"未入力"と表示
    * 入力後は、その週のその科目の正答率を表示"正答率⚪︎%"
  * 科目ごとに"今すぐ記録する"のボタンを配置。一度しか入力されていない場合で、かつ正答率が80%未満の場合は、強調表示をする。なお、クリックすると、スパーク機能へ遷移する。正答率80%以上または再入力後は、強調表示はしない。再クリックは可能とする。
* **特別モード**：土曜日12時以降
...
* 無色（記録なし）
* 薄（入力1科目 または 正答率80%科目が1科目）
* 中（入力2科目 または 正答率80%科目が2科目）
* 濃（入力3科目以上 または 正答率80%科目が3科目以上）
...
生徒は「AIコーチと対話しながら作成」または「自分で直接記入」を選択できる。

| 項目        | 入力方式      | 選択肢/範囲 | バリデーション  |
| --------- | --------- | ------ | -------- |
| **今回の思い** | AI対話または直接入力 | 最大300字 | 自動生成→編集可 または 自由記述 |

### 入力方法の選択

コース・組を選択後、以下の2つの選択肢を提示する：

| 選択肢 | 説明 | UI |
| ----- | ---- | --- |
| **AIコーチと話し始める** | 3ターンの対話で「今回の思い」を生成 | 目立つプライマリボタン |
| **自分で入力する** | フリーテキストで直接記入 | セカンダリボタン |

### フロー①：AIコーチと話し始める（3ターン）

```yaml
Step1_目標確認:
  AI: "○○さん、今回は[コース][組]を目指すんだね！"
  備考: AIコーチがモチベーションを高める一言を添える
  表示: 即座に表示
  遷移: 3秒後に自動でStep2へ
...
Step2_感情探索:
  ...
  生徒: 自由記述（最大80字）
  UI: 吹き出し式入力ボックス
  入力方法: Enterキーで改行、送信ボタンをクリック/タップで送信
...
* ChatGPTのAPIを利用。モデルはGPT-4o-mini
