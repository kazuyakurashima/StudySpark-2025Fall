...
- **復習促進モード**：**火 / 木 / 土**
  ...
  - パネル全体の右上に、入力状況を表示。例：2/3 完了、など
    ...
    - 火・木・土は科目ごとに以下の条件で完了とする：
      - **正答率が 80%以上に到達した科目**は完了
      - **正答率が 80%未満の科目**は、再入力して正答率が向上した場合のみ完了
      - 正答率が 80%未満で正答率が向上していない場合は未完了
  ...
  - 科目ごとに"今すぐ記録する"のボタンを配置。一度しか入力されていない場合で、かつ正答率が 80%未満の場合は、強調表示をする。なお、クリックすると、スパーク機能へ遷移する。正答率 80%以上または再入力後は、強調表示はしない。再クリックは可能とする。
...
コース・組を選択後、以下の 2 つの選択肢を提示する：

| 選択肢                    | 説明                                 | UI                     |
| ------------------------- | ------------------------------------ | ---------------------- |
| **AI コーチと話し始める** | 3 ターンの対話で「今回の思い」を生成 | 目立つプライマリボタン |
