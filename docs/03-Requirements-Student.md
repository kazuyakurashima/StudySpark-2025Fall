...
| 第６回公開組分けテスト | 10/5（日）  | 9/1〜10/5   |
| 第７回公開組分けテスト | 11/9（日）  | 10/1〜11/9  |
| 第８回公開組分けテスト | 12/14（日） | 11/1〜12/14 |
| 新６年公開組分けテスト | 1/25（日）  | 12/1〜1/25  |

## 小学 6 年生

| テスト名                | 実施日      | 表示期間    |
| ----------------------- | ----------- | ----------- |
| 第 3 回合不合判定テスト | 9/7（日）   | 8/1〜9/7    |
| 第 4 回合不合判定テスト | 10/5（日）  | 9/1〜10/5   |
| 第 5 回合不合判定テスト | 11/16（日） | 10/1〜11/16 |
| 第 6 回合不合判定テスト | 12/7（日）  | 11/1〜12/7  |
...
> **2026年度 DB設計変更に関する注記（2026-02-09）**
>
> 以下の階層フィルタ仕様（A→Aのみ, B→A+B, C/S→全件）は **2025年度の設計** です。
> 2026年度以降、`study_content_types` テーブルは各コース（A/B/C/S）ごとに専用の行を持つ設計に変更されました。
> アプリ側では `WHERE course = 生徒のコース` で必要な全コンテンツを取得するため、階層フィルタは不要です。
> 学習内容名・問題数もDBマスターデータ（`study_content_types` + `problem_counts`）が一次情報です。
> 以下の表と問題数は2025年度の参考情報として残しています。
...
> **注記**: 2026年度の DB 設計変更については「スパーク機能」セクション内の同名見出しの注記を参照してください。
