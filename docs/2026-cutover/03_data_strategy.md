...
| `units` | 単元マスタ | 2026年度版を投入 |
| `contents` | 学習内容マスタ | 2026年度版を投入 |
| `study_sessions` | 学習回マスタ | 2026年度版を投入（期間情報更新） |
| `assessment_masters` | テスト・プリントマスタ（マスタープリント[算数]・漢字テスト） | 2026年度版を投入 |
...
| `units` | カリキュラム変更があれば更新 |
| `contents` | 学習内容の追加・変更 |
| `assessment_masters` | マスタープリント（算数）・漢字テストのタイトル・配点等 |

### 8.2 投入方法

```
1. 2026年度版のマスタデータを CSV/JSON で準備
2. シードスクリプト（scripts/seed-2026-masters.ts 等）を作成
3. DB2026 に投入
4. 検証
```
