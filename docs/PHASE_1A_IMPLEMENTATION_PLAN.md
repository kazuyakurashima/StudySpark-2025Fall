...
### データ整備（D-2〜D-4）
1. **ユーザー**: 算数 小5 第5回（総合回）の問題データ提供 → 到達度マップの通常回/総合回2パターン検証
2. **ユーザー**: 第2回以降のデータ順次整備（通常回テンプレートで横展開）

### 後続機能
3. **開発者**: セルクリックで設問詳細（○×一覧）表示（到達度マップの拡張）
4. **開発者**: 通常回/総合回の列ブロック分割（第5回データ投入後）

### Phase 1A' → 1B → 1C
5. **Phase 1A'**: 小6算数（上）— スキーマ変更なし、データ追加のみ
6. **Phase 1B**: 理科・社会（小5+6同時）— 横展開
7. **Phase 1C**: 旧入力整理 — 予習シリーズ非表示化、AI分析入力源移行、ホーム画面進捗切替
