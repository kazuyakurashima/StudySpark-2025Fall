...
> **ステータス**: Phase 1 完了、Phase 1.5a 完了、Phase 2 完了、Phase 1.5b 計画策定済み
> **最終更新**: 2026-03-05
...
| **1.5b** | ゴールナビ動的化+SSE（Simple Step 1-3 / Full Step 1-2） | 🔵 計画v4承認済み |
| **1.5b** | Sparkフィードバック SSE化 | 🔵 計画v4承認済み |
...
| 8 | `app/api/goal/simple-navigation/route.ts` | 修正 | Steps 2-3をLLM動的化（非ストリームフォールバック経路） |
| 9 | `app/student/goal/goal-simple-chat.tsx` | 修正 | `fetchSSE` 消費 + Step4 `simulateTyping` |
| 10 | `app/student/goal/goal-navigation-chat.tsx` | 修正 | `fetchSSE` 消費 |
| 11 | テスト（3ファイル） | **新規** | typing-effect(5), fetchSSE(6), output-validator(8) |
...
3. **出力バリデーション**: Steps 2-3 の done 時点で品質チェック → 失敗時は `meta: replaced_by_template` + フォールバックテンプレートで done 送出
...
| `lib/openai/__tests__/goal-output-validator.test.ts` | validateGoalStepOutput | 8 |
| `lib/services/__tests__/coach-feedback.test.ts` | サービス層 | 4 |

#### 実装ブランチ戦略

```
feature/phase-1.5b-plan-doc      ← 本ドキュメント更新（レビュー単位1）
feature/phase-1.5b-goal-backend  ← A. ゴールナビ バックエンド（Step 1-6）
feature/phase-1.5b-goal-frontend ← A. ゴールナビ フロントエンド（Step 7-8）
feature/phase-1.5b-spark-sse     ← B. Sparkフィードバック SSE（Step 9-12）
```

各ブランチで `pnpm test` / `pnpm build` を通して小さくPR。
...
**Sparkフィードバック**:
- [ ] フィードバックがSSEストリーミングで表示される
- [ ] done → meta(save_ok/save_failed) → close の順序が守られている
- [ ] キャッシュヒット時も SSE（即 done）で返される
- [ ] `meta: save_failed` 時に「再保存」ボタンが表示される
- [ ] サービス層がRoute Handler / Server Action両方から利用されている
