...
> **ステータス**: Phase 1 完了、Phase 1.5a 完了、Phase 2 完了、Phase 1.5b Full flow完了・Simple導線未実装
> **最終更新**: 2026-03-05
...
| **1.5b** | ゴールナビ Simple flow 導線実装（page.tsx接続） | 🔴 未実装（別チケット） |
| **1.5b** | 互換レイヤー削除（Simple導線完了後） | ⬜ 未着手 |
...
PR #19（goal-frontend: Full flowのみ）→ ✅ merged
  ...
  ↓
別チケット: GoalSimpleChat 導線実装（page.tsx接続 + Simple flow UI）
  ↓
互換レイヤー削除（上記完了 + 本番旧ペイロード0%確認後）
  ↓
spark-sse PR 作成
```
...
feature/phase-1.5b-simple-routing ← A. GoalSimpleChat 導線実装（🔴 別チケット）
...
- 互換レイヤー削除は Simple flow 導線チケット完了後に実施
