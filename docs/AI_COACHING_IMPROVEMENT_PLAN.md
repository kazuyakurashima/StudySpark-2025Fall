...
> **ステータス**: Phase 1 完了、Phase 1.5a 完了、Phase 2 完了、Phase 1.5b 実装監査v5反映中
> **最終更新**: 2026-03-05
...
| **1.5b** | ゴールナビ動的化+SSE（Simple Step 1-3 / Full Step 1-2） | 🟡 実装監査v5反映中（PR #17 Draft） |
...
- **goal-frontend PR**: フロント側を `testScheduleId` 送信に切替え、互換レイヤーを削除
- **マージ順序**: goal-backend（互換レイヤー付き）→ goal-frontend（互換レイヤー削除）の順序厳守
...
feature/phase-1.5b-v5-plan-doc    ← 実装監査v5ドキュメント更新（レビュー中）
feature/phase-1.5b-goal-backend   ← A. ゴールナビ バックエンド（PR #17 Draft凍結中）
feature/phase-1.5b-goal-frontend  ← A. ゴールナビ フロントエンド
feature/phase-1.5b-spark-sse      ← B. Sparkフィードバック SSE
feature/phase-1.5b-rescue         ← WIP救済ブランチ（参照用、マージしない）
```

**マージ順序**: v5-plan-doc → goal-backend → goal-frontend → spark-sse
**運用ルール**:
- 各ブランチは毎回 main 最新から作る
- rescue からは必要ファイルだけ取り込む（丸ごと cherry-pick しない）
- 各PRで `pnpm test` / `pnpm build` を通す
- goal-backend は互換レイヤー付きでマージ → goal-frontend で互換レイヤー削除
