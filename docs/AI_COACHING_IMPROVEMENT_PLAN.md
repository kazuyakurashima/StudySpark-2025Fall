...
36. **gemini-utils.ts共有化**: `buildGeminiContents()` を `lib/llm/gemini-utils.ts` に抽出、reflect-coaching.ts のローカル実装を削除
...
40. **goal-coaching.ts**: 多ターン対話（stepPrompt先頭 + history）+ JSON出力（responseMimeType）
41. **encouragement.ts**: JSON出力 + キャッシュ。2関数（messages/suggestions）のprovider分岐
42. **coach-feedback.ts**: `callLLM()` ヘルパー方式。Langfuse metadata に provider 追加、AbortSignal対応
43. **simple-navigation/simple-thoughts route**: `new OpenAI()` 直接使用を `getOpenAIClient()`/`getGeminiClient()` に置換
44. **sanitizeForLog()**: 全変更ファイルの console.error に適用
