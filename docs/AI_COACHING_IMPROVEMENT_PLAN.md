...
8. **[FB3] 冪等性 (request_id)** — リトライ時の二重保存対策
...
| `app/student/reflect/reflect-chat.tsx` | 修正 | ストリーム消費 + deltaバッチ更新 + AbortController + request_id |
...
**A) [FB3] request_id付きストリーム消費 + deltaバッチ更新:**
...
const sendWithStreaming = async (requestBody: object, nextTurn: number) => {
  ...
  try {
    ...
    // [FB3] request_idでDB側の重複チェック（saveCoachingMessage側で対応）
