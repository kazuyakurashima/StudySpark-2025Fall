...
### P0-10: 保護者新規登録UI実装 ⏳ 未着手 (0/4完了)

- [ ] 保護者新規登録ページ作成 (`app/register/parent/page.tsx`)
  - 対応要件: `02-Requirements-Auth.md`
  - 内容: 保護者情報入力フォーム（メール、パスワード、氏名、子どもの学習ID）
  - 実装: 既存のparentSignUp Server Actionを使用
  - UI要素:
    - 保護者メールアドレス入力
    - パスワード入力（確認用含む）
    - 保護者氏名入力
...
- [ ] フォームバリデーション実装
  - 対応要件: UX要件
  - 内容: react-hook-form + zod でバリデーション
  - 検証項目:
    - メールアドレス形式チェック
    - パスワード強度チェック（8文字以上）
    - パスワード確認一致チェック
...
- [ ] 既存生徒との紐付けロジック実装
  ...
  - 実装: parentSignUp Server Action内で処理済み（確認のみ）

- [ ] 登録完了後のフロー実装
  - 対応要件: UX要件
  - 内容: 登録成功 → メール確認案内 → ログインページへリダイレクト
  - 実装: 成功メッセージ表示、自動ログイン or ログインページ遷移
