...
- [x] 生徒詳細画面実装 ✅ **DB連携完了（2025-10-11）**
  - 対応要件: `05-Requirements-Coach.md`
  - 検証: 各生徒カードに6つのアクション（ホーム/ゴールナビ/達成マップ/学習履歴/応援履歴/コーチング履歴）
  - 実装: UI実装 + DB連携完了（`app/coach/student/[id]/page.tsx`）
  - Server Actions: `getStudentDetail()`, `getStudentLearningHistory()`, `sendEncouragementToStudent()`
  - 統計データ: 連続日数（RPC）、週リング（今週の学習数）、最新スコア（直近10件平均）
  - AI応援: API Route経由でメッセージ生成 (`/api/coach/encouragement-suggestions`)
...
- [x] 指導者が各生徒の詳細情報を閲覧できる ✅ **完了（2025-10-11 DB連携実装完了）**
...
- ✅ 生徒詳細画面 (`app/coach/student/[id]/page.tsx`) - **DB連携完了（2025-10-11）**
