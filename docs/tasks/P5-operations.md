...
### P5-2: データ削除ポリシー実装 🔄 進行中 (1/4)

- [x] 削除ロジック実装 ✅ **完了（関数のみ）**
  - 対応要件: データ保持ポリシー
  - 実装: `supabase/migrations/20251004000008_create_audit.sql`
    - `cleanup_old_audit_logs()` - 1年以上前の監査ログを削除
    - `cleanup_old_ai_cache()` - 30日以上アクセスされていないキャッシュを削除
    - `cleanup_old_weekly_analysis()` - 6週間以上前の週次分析を削除
    - `cleanup_old_notifications()` - 期限切れまたは60日以上前の既読通知を削除
    - `run_data_retention_cleanup()` - 上記すべてを一括実行するマスター関数
  - **状況**: 関数は実装済みだが、**自動実行の仕組みが未設定**
...
- [ ] 削除バッチ処理実装（スケジューラ設定） ⏳ **未着手**
  - 対応要件: 運用要件
  - 検証: 毎週日曜0時に自動実行、削除件数ログ記録
  - **状況**: 削除関数は実装済み、スケジューラ未設定
  - **必要な作業**:
    - **Option 1**: Vercel Cron でエンドポイント作成
      - `/app/api/cron/data-retention/route.ts` 作成
      - `vercel.json` に週次スケジュール追加
    - **Option 2**: Supabase pg_cron 拡張を使用
      - `SELECT cron.schedule('data-retention', '0 0 * * 0', 'SELECT run_data_retention_cleanup()');`
    - **Option 3**: GitHub Actions で週次実行
  - **推奨**: Option 1（Vercel Cron）- 既存のバッチ処理と統一

- [ ] 削除結果のログ記録 ⏳ **未着手**
  - 対応要件: 運用要件
  - 検証: 削除件数を `cleanup_logs` テーブルに記録
  - **必要な作業**:
    - `cleanup_logs` テーブル作成
    - 削除バッチ実行時にログ保存
...
**現在:** 🔄 Phase 5 進行中（バックエンド23%完了）
**次:** P5-1 監査ログ閲覧UI実装、または P5-2 削除バッチスケジューラ設定

**推奨実装順:**
1. **P5-2 削除バッチスケジューラ設定** - 既存の関数を動かすだけ（工数小）
2. **P5-3 管理者ダッシュボード作成** - 他のUI実装の基盤
3. **P5-1 監査ログ閲覧UI** - 管理者ダッシュボードの一部
4. **P5-4 Sentry統合** - エラー追跡強化
5. **P5-5 総合テスト** - すべての機能確認
