...
進み具合の詳細は以下の階層で確認できます：

1. **このREADME** – 最新トピックと本番ステータスのハイライト
2. **[docs/tasks/PROJECT-PROGRESS.md](docs/tasks/PROJECT-PROGRESS.md)** – フェーズ別の進捗率とタスク完了数
3. **[docs/tasks/TASK_MANAGEMENT.md](docs/tasks/TASK_MANAGEMENT.md)** – 各フェーズのタスク一覧とチェックリスト

> 🎉 **最新の成果 (2025-11-20):**
> - **リフレクトコーチングUX改善** - 終了ボタン表示ロジック改善、折り畳み式続行入力欄追加
> - **認証情報セキュリティ強化** - 実メールアドレス・パスワードの匿名化完了
> - **投資家向けデモ対応** - 期限付きデモアカウント表示（11/28まで）
...
- **デモアカウント:** `akira5` / **パスワード:** `<社内管理>`（星野明・小5・Bコース）
- **デモアカウント:** `hikaru6` / **パスワード:** `<社内管理>`（星野光・小6・Aコース）
- **追加テストアカウント:** その他17名（ログインID: `mao5`, `ことのか5`, `いち5`, `はるき5`, `ななこ5`, `ともき5`, `しゅうへい5`, `たくみ6`, `たいよう6`, `としたか6`, `みやこ6`, `しょうや6`, `まなと6`, `ともえ6`, `みすず6`, `そうま6`）パスワードは別途管理

#### 指導者（3名）
- **開発用:** `kazuya@studyspark.jp` / **パスワード:** `<社内管理>`（倉島先生・全生徒担当）
- **テスト用:** `nakatani@studyspark.jp` / **パスワード:** `<社内管理>`（中谷先生・全生徒担当）
- **デモ用:** `demo@studyspark.jp` / **パスワード:** `<社内管理>`（デモ指導者・3名担当）

#### 管理者（1名）
- **システム管理:** `admin@studyspark.jp` / **パスワード:** `<社内管理>`

> **注記:** 指導者・管理者アカウントは招待制ですが、POC段階では手動作成しています。招待機能は将来実装予定です。
...
全テーブルでRLSが有効化されており、ロールに応じたデータアクセス制御を実現：
- **生徒**: 自分のデータのみ
- **保護者**: 自分のデータ + 子どものデータ
- **指導者**: 自分のデータ + 担当生徒のデータ
- **管理者**: 全データ
