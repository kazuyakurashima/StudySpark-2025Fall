...
async function testAssessmentHistory() {
  ...
  assessments.forEach((a: any, i: number) => {
    const percentage = a.max_score_at_submission > 0
      ...
      : 0
    console.log(`   ${i + 1}. ${a.assessment_masters?.assessment_name} - ${a.score}/${a.max_score_at_submission} (${percentage}%)`)
  })
  ...
  // 算数プリントのみフィルター
  const mathOnly = allHistory?.filter((a: any) => a.assessment_masters?.assessment_type === 'math_print') || []
  console.log(`✅ 算数プリント: ${mathOnly.length}件`)

  // 漢字テストのみフィルター
  const kanjiOnly = allHistory?.filter((a: any) => a.assessment_masters?.assessment_type === 'kanji_test') || []
  console.log(`✅ 漢字テスト: ${kanjiOnly.length}件`)

  // 4. getAssessmentSummary() のシミュレーション
  console.log('\n📌 Step 4: getAssessmentSummary() をシミュレート...')

  const mathAssessments = allHistory?.filter((a: any) => a.assessment_masters?.assessment_type === 'math_print') || []
  const kanjiAssessments = allHistory?.filter((a: any) => a.assessment_masters?.assessment_type === 'kanji_test') || []
  ...
  if (latestMath) {
    const mathPercentage = latestMath.max_score_at_submission > 0
      ...
      : 0
    console.log(`  📊 算数プリント: ${(latestMath as any).assessment_masters?.assessment_name}`)
  ...
  if (latestKanji) {
    const kanjiPercentage = latestKanji.max_score_at_submission > 0
      ...
      : 0
    console.log(`  ✏️  漢字テスト: ${(latestKanji as any).assessment_masters?.assessment_name}`)
